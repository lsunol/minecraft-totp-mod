 ...
 *
 * <p>This is intentionally in-memory only: nothing here is persisted, so every
 * player must re-authenticate on every join. Reads happen from the network
 * thread (via the packet-handler mixin), hence the concurrent set.</p>
 */
